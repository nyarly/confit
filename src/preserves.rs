...
use datasource::{union, REFS, REMOTE, STATUS};
...
impl Check {
    ...
    pub fn tagged_checks<'a, 'b>(
        tags: impl Clone + IntoIterator<Item = &'b str>,
    ) -> Vec<&'a Check> {
        ALL_CHECKS
            .iter()
            .filter(move |ch| {
                tags.clone()
                    .into_iter()
                    .any(|t| ch.tags.iter().any(|&c| t == c))
            })
            .collect()
    }

    pub fn all_tags() -> Vec<&'static str> {
        let mut tags = ALL_CHECKS
            .iter()
            .flat_map(|ch| ch.tags.iter().copied())
            .collect::<Vec<_>>();
        tags.sort_unstable();
        tags.dedup();
        tags
...
impl CheckList for Vec<&Check> {
    fn required_sources(&mut self) -> datasource::Group {
        self.iter()
            .fold(datasource::EMPTY, |acc, check| acc | check.required_data)
    }
}
...
impl<'a> Summary<'a> {
    ...
    pub fn exit_status(&self) -> i32 {
        self.items().iter().fold(0, |status, item| {
            if item.passed {
                status
            } else {
...
fn remote_changes(s: &Summary) -> CheckResult {
    s.status
        ...
        .clone()
        .map_or(1, |b| {
            b.commits.map_or(1, |TrackingCounts(_, behind)| behind)
        })
        .into()
}
...
fn unpushed_commit(s: &Summary) -> CheckResult {
    s.status
        .branch
        .clone()
        .map_or(1, |b| b.commits.map_or(1, |TrackingCounts(ahead, _)| ahead))
        .into()
}
